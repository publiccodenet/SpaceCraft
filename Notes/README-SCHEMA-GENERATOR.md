...
FILE PATH: Unity/SpaceCraft/Assets/Editor/SchemaGenerator/README.md
...
* GENERATED FILES PATH: Unity/SpaceCraft/Assets/Scripts/Schemas/Generated/
...
3. ONLY THEN regenerate all schemas using SpaceCraft > Import All Schemas
...
4.  **Generate C# Code:** In Unity, use the `SpaceCraft > Schema Generator` window or run `npm run unity:regenerate-schemas` from BackSpace. The `SchemaGenerator.cs` script reads the processed `*.json` schema files **directly** from `StreamingAssets/Content/schemas`.
...
4. **WebGL-Specific JSON.NET Reader Issue**: There's a critical WebGL-specific behavior with JsonReader:
   - In WebGL builds, when using `CreateReader()` on a JToken, the reader starts with `TokenType.None` but still contains valid data
   - In native builds, the reader's initial position is set correctly
   - **Required Workaround**: Always call `reader.Read()` before using any JsonReader in converters:
   ```csharp
   // CORRECT PATTERN - Always do this:
   var reader = json["propertyName"].CreateReader();
   reader.Read(); // Move to first token - CRITICAL for WebGL!
   converter.ReadJson(reader, typeof(string), null, null);
   
   // WRONG PATTERN - Will fail in WebGL:
   converter.ReadJson(json["propertyName"].CreateReader(), typeof(string), null, null);
   ```
   - This issue is not documented by Unity or Newtonsoft, but causes silent failures in WebGL builds
   - Our schema generator handles this automatically for all converter-based properties
...
3.  Ensure exported `*.json` files are present in `Unity/SpaceCraft/Assets/StreamingAssets/Content/schemas`.
4.  Open the Unity generator window: `SpaceCraft > Schema Generator`.
