...
public class Accessor {
        ...
        for (int stepIndex = 0, stepCount = steps.Length;
            ...
            switch (prefix) {
                ...
                case "list": // List index
                    ...
                    if (searchJArray && (nextObj is JArray)) {
                    ...
                    } else if (searchArray && (nextObj.GetType().IsArray)) {
                        ...
                        Debug.LogError("Accessor: FindAccessor: prefix: " + prefix + " expected indexed array or list nextObj: " + nextObj + " type: " + nextObj.GetType() + " is array: " + (nextObj is Array) + "path: " + path + " searchArray: " + searchArray + " searchList: " + searchList + " searchJArray: " + searchJArray);
                        accessor = null;
                        return false;
                    }
                ...
                case "id": // id of object
                    ...
                    accessor.Init_Id(nextObj, rest, conditional, excited);
    ...
    public static bool SetProperty(object target, string name, JToken jsonValue)
    {
        ...
        if (accessor.excited) {
            ...
            if (!BridgeJsonConverter.ConvertToType(jsonValue, targetType, ref value)) {
                if (!accessor.conditional) {
    ...
    public bool Get_Component(ref object result)
    {
        ...
        if (componentType == null) {
            componentType = FindTypeInLoadedAssemblies("Bridge." + str);
            //Debug.Log("Accessor: Get_Component FindTypeInLoadedAssemblies str: Bridge." + str + " componentType: " + componentType);
        }
    ...
    {
        string id = Bridge.bridge.GetID(obj);
        result = id;
        //Debug.Log("Accessor: Get_Id: str: " + str + " obj: " + obj + " `id: " + id);
        return true;
    }
    ...
    public bool Set_Method(object obj, object value)
    {
        ...
        for (int i = staticStart, n = parameterInfos.Length; i < n; i++) {
            ...
            bool success = BridgeJsonConverter.ConvertToType(jsParameter, parameterType, ref val);
            if (!success) {
    ...
    public Type GetTargetType()
    {
        switch (type) {
            ...
            case AccessorType.JArray:
                return typeof(JToken); // XXX should this be JArray or jToken?
            ...
            case AccessorType.JObject:
                return typeof(JToken); // XXX should this be JObject or jToken?
