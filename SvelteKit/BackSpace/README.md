...
The SpaceCraft Content Pipeline manages the workflow of content from sources (like Internet Archive) to Unity's StreamingAssets directory. It follows a modular architecture with three main phases:

1. **Bootstrap**: Generate a skeleton whitelist from existing Unity content
2. **Pull**: Fetch content from sources to a shared cache
3. **Export**: Transfer filtered content to Unity
...
Key npm commands:
```bash
npm run pipeline:run        # Run complete pipeline
npm run schema:generate-all # Generate schemas for Unity
```
